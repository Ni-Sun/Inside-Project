...
//////////////////////////////////////////////////////////////////////////////////	 
//???????????????δ????????????????????????κ????
//ALIENTEK ???STM32F103??????V3
//2.4??/2.8??/3.5??/4.3??/7?? TFT???????	  
//???????IC??????:ILI9341/ILI9325/RM68042/RM68021/ILI9320/ILI9328/LGDP4531/LGDP4535/
...
//********************************************************************************
//V1.2??????
//?????SPFD5408??????,????????ID???????HEX???.?????LCD????IC.
//V1.3
//?????????IO?????
//??????????????????????V1.8???????????汾??
//????1.8?汾??(??????1.8)????????,?????LCD_Init??????LCD_LED=1;?LCD_LED=1;
//V1.4
//?????LCD_ShowChar????????????????????
//???????????????????
//V1.5 20110730
...
//2,????LCD_Display_Dir????,?????????????л?
//V1.9 20120911
//1,????RM68042??????ID:6804????????6804??????????????????????跽???
//????6804?????????Ч???????????????У???????
//V2.0 20120924
//????????λ???????,ILI9341??ID??????????9300,???LCD_Init,????????
//???????????ID?9300/???ID??,??????????IC?ILI9341?????9341????????
//V2.1 20120930
//????ILI9325???????bug??
//V2.2 20121007
//????LCD_Scan_Dir??bug??
...
//V2.4 20131120
//1,????NT35310??ID:5310?????????????
//2,????LCD_Set_Window????,???????????,????????,???????,????ú?????????,?????6804.
//V2.5 20140211
//1,????NT35510??ID:5510?????????????
//V2.6 20140504
//1,????ASCII 24*24????????(???????????????????????)  
//2,?????????????,?????MDK -O2???
//3,???9341/35310/35510,д???????????,?????????????
//4,?????SSD1289?????,???1289??????????,???????1us...???????.?????F4???
//5,????68042??C505??IC????????????bug.
//V2.7 20140710
//1,????LCD_Color_Fill?????????bug. 
//2,????LCD_Scan_Dir?????????bug.
//V2.8 20140721
//1,???MDK???-O2????LCD_ReadPoint?????????Ч??????.
//2,????LCD_Scan_Dir????????????跽?????????bug.
//V2.9 20141130
//1,??????SSD1963 LCD?????.
...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	if(lcddev.id==0X1963)return r;					//1963????????? 
	opt_delay(2);	  
 	r=LCD_RD_DATA();  		  						//??????????
 	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510)		//9341/NT35310/NT35510???2?ζ???
 	{
		opt_delay(2);	  
		b=LCD_RD_DATA(); 
		g=r&0XFF;		//????9341/5310/5510,????ζ??????RG???,R???,G???,???8λ
		g<<=8;
	} 
	if(lcddev.id==0X9325||lcddev.id==0X4535||lcddev.id==0X4531||lcddev.id==0XB505||lcddev.id==0XC505)return r;	//????IC??????????
...
//????,????????L2R_U2D????,??????????????跽?,????????????????.
//dir:0~7,????8??????(???嶨???lcd.h)
//9320/9325/9328/4531/4535/1505/b505/5408/9341/5310/5510/1963??IC?????????	   	   
...
{
    u16 original_color = POINT_COLOR;  // 保存原始颜色
    POINT_COLOR = Color;               // 设置新颜色
    LCD_DrawLine(x1, y1, x2, y2);      // 调用系统画线函数
    POINT_COLOR = original_color;      // 恢复原始颜色
}
...
void LCD_Display_Dir(u8 dir)
{
	if(dir==0)			//????
	{
		...
		{
			lcddev.wramcmd=0X2C;	//????д??GRAM????? 
			lcddev.setxcmd=0X2B;	//????дX???????
			lcddev.setycmd=0X2A;	//????дY???????
...
{ 					
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOE|RCC_APB2Periph_GPIOG,ENABLE);//???PORTB,D,E,G???AFIO???ù??????
...
*/
  ...
  readWriteTiming.FSMC_AddressHoldTime = 0x0f;	 //??????????ADDHLD????Aδ???	
  ...
  writeTiming.FSMC_AddressHoldTime = 0x0f;	 //??????????A		
  ...
  FSMC_NORSRAMInitStructure.FSMC_WriteOperation = FSMC_WriteOperation_Enable;	//  ?洢??д???
  FSMC_NORSRAMInitStructure.FSMC_WaitSignal = FSMC_WaitSignal_Disable;   
  FSMC_NORSRAMInitStructure.FSMC_ExtendedMode = FSMC_ExtendedMode_Enable; // ??д??ò???????
  FSMC_NORSRAMInitStructure.FSMC_WriteBurst = FSMC_WriteBurst_Disable; 
  FSMC_NORSRAMInitStructure.FSMC_ReadWriteTimingStruct = &readWriteTiming; //??д???
  FSMC_NORSRAMInitStructure.FSMC_WriteTimingStruct = &writeTiming;  //д???
  ...
  FSMC_NORSRAMInit(&FSMC_NORSRAMInitStructure);  //?????FSMC????
	...
	delay_ms(50); 					// delay 50 ms 
  	...
  	if(lcddev.id<0XFF||lcddev.id==0XFFFF||lcddev.id==0X9300)//????ID?????,????lcddev.id==0X9300?ж?????9341??δ????λ????????????9300
	{	
 		//????9341 ID????		
		LCD_WR_REG(0XD3);				   
  ...
  //LCD_direction(USE_HORIZONTAL);//????LCD???????
	...
	}else if(lcddev.id==0x9328)//ILI9328   OK  
	{
        ...
        delay_ms(50); 
        LCD_WriteReg(0x0050,0x0000); //??GRAM???λ?? 
        LCD_WriteReg(0x0051,0x00ef); //??GRAM???λ??                    
        LCD_WriteReg(0x0052,0x0000); //???GRAM???λ??                    
        LCD_WriteReg(0x0053,0x013f); //???GRAM???λ??  
	...
	}else if(lcddev.id==0x5408)
	{
		...
		LCD_WriteReg(0x50,0x0000); //???ò????????X?????
		LCD_WriteReg(0x51,0x00EF); //???ò????????X???????
		LCD_WriteReg(0x52,0x0000); //???ò????????Y?????
		LCD_WriteReg(0x53,0x013F); //???ò????????Y???????
		LCD_WriteReg(0x60,0x2700); //Driver Output Control
		//????????????????????????
		LCD_WriteReg(0x61,0x0001); //Driver Output Control
...
//????????void fWriteHz24(u8 *hzP,u16 v16PosX,u16 v16PosY,u8 v8Zoom,u16 v16Color)
//????????????? v16PosX,v16PosY?????????????v16Color?????д??????????v8Zoom
//????????????
//?????????
//?????????
//?????????
...
void fWriteHz24(u8 *hzP,u16 v16PosX,u16 v16PosY,u8 v8Zoom,u16 v16Color)
{
	...
	for (v8Row = 0 ; v8Row < 24; v8Row ++) //24??
	{
		for(v8Col = 0 ; v8Col < 3 ; v8Col ++)//??? 3?????
		{
			v8HzData = *hzP++;
			for(v8Index = 0 ; v8Index < 8 ; v8Index ++)//д??????????????8??
			{
